...
const STATUS = {
  ...
  unsaved: { text: '● Non sauvegardé', color: '#f90' },
...
export default function DubbingWorkspace({ clip, onUpdate, onBack }) {
    ...
    function draw() {
          ...
          if (waveData && waveData.samples.length > 0) {
            ...
            if (isNeon) {
              ...
              for (let si = 0; si < samples.length; si++) {
                ...
                const amp = (samples[si] * 2 - 1) * maxAmp * 0.6
                if (!started) { ctx.moveTo(sx, yMid + amp); started = true } else { ctx.lineTo(sx, yMid + amp) }
              }
              ctx.stroke()
              ctx.shadowBlur = 0
            } else {
              ctx.fillStyle = color.bg.replace('0.10', '0.20')
              for (let si = 0; si < samples.length; si++) {
                const sampleTime = si * timePerSample
                const sx = Math.round(cursor_x + (sampleTime - t) * pxSec)
                if (sx < 0 || sx > W) continue
                const amp = Math.max(1, samples[si] * maxAmp)
                ctx.fillRect(sx, yMid - amp, 1.5, amp * 2)
              }
            }
          } else {
            // Synthetic animated sine wave fallback
            const baseAlpha = isNeon ? 0.7 : 0.25
            ctx.strokeStyle = isNeon
              ? color.label
              : color.label.replace(')', `,${baseAlpha})`).replace('rgb', 'rgba')
            ctx.lineWidth = isNeon ? 1.2 : 1
            if (isNeon) { ctx.shadowColor = color.label; ctx.shadowBlur = 4 }
            ctx.beginPath()
            let started = false
      ...
      for (const sub of subtitlesRef.current) {
        ...
        if (sub.text) {
          ...
          const BASE_FONT = Math.max(14, Math.round(22 * fontScaleRef.current))
          ...
          const scaleY = Math.max(1, (trackH2 * 0.52) / BASE_FONT)

          const baseFill = isActive ? '#fff' : 'rgba(255,255,255,0.72)'
          if (isNeon) {
            ctx.shadowColor = color.label
      ...
      if (overlayCanvas) {
        ...
        for (let tr = 0; tr < nTracks; tr++) {
          ...
          for (let px = 0; px <= oW; px += 2) {
            ...
            if (waveData && waveData.samples.length > 0) {
              ...
              amp = (si >= 0 && si < waveData.samples.length) ? (waveData.samples[si] * 2 - 1) * maxAmp * 0.6 : 0
            } else {
              amp = (Math.sin(sampleT * 7 + tr * 1.3) * 0.6 + Math.sin(sampleT * 23 + tr * 0.7) * 0.3) * maxAmp * 0.55
            }
        ...
        for (const sub of subtitlesRef.current) {
          ...
          if (sub.text) {
            const O_BASE_FONT = Math.max(10, Math.round(14 * fontScaleRef.current))
            ...
            const oScaleY = Math.max(1, (oTrackH * 0.52) / O_BASE_FONT)
            oc.save()
            oc.beginPath()
            ...
            let cx2 = 0
            for (const seg of segs) {
              oc.fillStyle = seg === '*' ? (isActive ? '#7ec0ff' : 'rgba(126,192,255,0.72)') : (isActive ? '#fff' : 'rgba(255,255,255,0.72)')
        ...
        // Cursor
        oc.strokeStyle = '#f90'; oc.lineWidth = 2; oc.setLineDash([])
        oc.beginPath(); oc.moveTo(oCursorX, 0); oc.lineTo(oCursorX, oH); oc.stroke()
        oc.fillStyle = '#f90'; oc.beginPath()
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':  e.preventDefault(); v.currentTime = Math.max(0, t - (e.shiftKey ? 0.1 : 5)); break
        case 'ArrowRight': e.preventDefault(); v.currentTime = Math.min(v.duration || 0, t + (e.shiftKey ? 0.1 : 5)); break
  ...
  function handleSubtitlesChange(subs) {
    ...
    debounceRef.current = setTimeout(() => doSave(subs), 1500)
  }
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '7px 14px', borderBottom: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <button onClick={transcribe} disabled={transcribing} style={{
          ...
          color: subtitles.length > 0 ? 'var(--text2)' : '#f90',
          border: `1px solid ${subtitles.length > 0 ? 'var(--border2)' : 'rgba(255,153,0,0.4)'}`,
        ...
        <button onClick={() => setShowExport(v => !v)} style={{
          padding: '4px 12px',
          background: showExport ? '#f90' : 'var(--surface3)',
          color: showExport ? '#000' : 'var(--text2)',
          border: `1px solid ${showExport ? '#f90' : 'var(--border2)'}`,
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ width: sidebarWidth, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid #1a1a1a' }}>
          {subtitles.length === 0 && !transcribing ? (
            <div style={{ padding: 24, textAlign: 'center' }}>
              ...
              <button onClick={transcribe} style={{ padding: '6px 16px', background: '#f90', color: '#000', fontWeight: 600, borderRadius: 4, fontSize: 12 }}>◉ Transcrire</button>
            </div>
          ) : (
            <SubtitleEditor subtitles={subtitles} onChange={handleSubtitlesChange} currentTime={currentTime} onSeek={seekTo} selectedIdx={selectedIdx} setSelectedIdx={setSelectedIdx} />
          )}
        </div>
        ...
        <div
          ...
          onMouseEnter={e => { e.currentTarget.style.background = '#f904' }}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          {/* Transport */}
          <div style={{ background: '#070707', borderTop: '1px solid #111', padding: '5px 12px', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <button onClick={togglePlay} style={{ background: 'none', color: '#f90', padding: '3px 7px', border: '1px solid #f904', borderRadius: 3, minWidth: 28, lineHeight: 1, fontSize: 12 }}>
              {playing ? '⏸' : '▶'}
            </button>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text2)', minWidth: 76, flexShrink: 0 }}>
              {fmt(currentTime)} / {fmt(duration)}
            </span>
            <input type="range" min={0} max={duration || 1} step={0.05} value={currentTime}
              onChange={e => seekTo(parseFloat(e.target.value))}
              style={{ flex: 1, accentColor: '#f90', cursor: 'pointer', height: 3 }} />
            <select value={speed} onChange={e => changeSpeed(e.target.value)}
              style={{ background: 'var(--surface3)', color: 'var(--text2)', border: '1px solid var(--border2)', borderRadius: 3, padding: '2px 4px', fontSize: 10 }}>
              {[0.5, 0.75, 1, 1.25, 1.5].map(s => <option key={s} value={s}>{s}×</option>)}
            </select>
            <button onClick={toggleMute}
              style={{ background: 'none', color: muted ? '#333' : 'var(--text2)', fontSize: 11, padding: '2px 6px', border: '1px solid var(--border2)', borderRadius: 3 }}>
              {muted ? '🔇' : '🔊'}
            </button>
          </div>
        </div>
      ...
      <div style={{ flexShrink: 0, borderTop: '2px solid #1a1a1a', background: '#050505', height: brPanelHeight, display: 'flex', flexDirection: 'column', overflow: 'visible', position: 'relative' }}>
        ...
        <div style={{ height: BR_CONTROLS_H, display: 'flex', alignItems: 'center', gap: 10, padding: '0 10px', borderBottom: '1px solid #111' }}>
          ...
          <div style={{ flex: 1 }} />
          {loopRegion && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', background: 'rgba(255,153,0,0.08)', border: '1px solid rgba(255,153,0,0.3)', borderRadius: 3 }}>
              <span style={{ fontSize: 11, color: '#f90', fontFamily: 'var(--font-mono)' }}>
                ⟲ {fmt(loopRegion.start)}–{fmt(loopRegion.end)}
              </span>
              <button onClick={() => setLoopRegion(null)} style={{ fontSize: 11, color: '#f90', background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
            </div>
          )}
        </div>
        {/* Canvas */}
            ...
            return (
              <input
                ...
                style={{
                  ...
                  border: '2px solid #f90', borderRadius: 4,
      ...
      {/* ── Export panel ── */}
      {showExport && (
        <div style={{ borderTop: '2px solid #f90', background: 'var(--surface)', padding: 14, flexShrink: 0, maxHeight: 260, overflow: 'auto' }}>
          <ExportPanel segmentId={clip.clip_id} subtitles={subtitles} />
        </div>
      )}
...
function BandeRythmoToolbar({
  ...
  const btn = (enabled = true, on = false, danger = false) => ({
    ...btnBase,
    color: !enabled ? '#3a3a3a' : (danger ? '#ff5a5a' : (on ? '#f90' : 'var(--text)')),
    background: on ? 'rgba(255,153,0,0.13)' : 'transparent',
  ...
  return (
    <div ref={toolbarRef} style={{
      ...
      <button style={{
        ...
        background: '#1a1a1a', border: `1px solid ${cColor}55`,
        color: cColor, fontSize: 12, fontWeight: 600, borderRadius: 8,
      }}>
        <span style={{
          width: 18, height: 18, borderRadius: '50%', background: cColor + '33',
          border: `1px solid ${cColor}`, display: 'inline-flex', alignItems: 'center',
          justifyContent: 'center', fontSize: 10, color: cColor, fontWeight: 700,
        }}>
      ...
      {/* Mic / record */}
      <button onClick={() => setRecording(r => !r)} title="Enregistrer une prise"
        style={{
          ...btnBase, height: 34, width: 34, minWidth: 34, padding: 0,
          background: recording ? '#e54545' : '#1a1a1a',
          border: `1px solid ${recording ? '#ff7575' : '#e5454555'}`,
          color: recording ? '#fff' : '#e54545', borderRadius: 8,
        }}>
        <Ic d={ICONS.mic} />
      </button>
      ...
      <div style={grp}>
        ...
        <button onClick={togglePlay} title={playing ? 'Pause' : 'Lecture'}
          style={{ ...btn(true), background: '#f90', color: '#000', minWidth: 40, height: 30, margin: '0 2px' }}>
      ...
      <div style={grp}>
        ...
        <button title="Marque de coupe" style={btn(target != null)}><Ic d={ICONS.scissors} /></button>
      </div>
      ...
      <div style={grp}>
        ...
        <button onClick={() => setLocked(l => !l)} title={locked ? 'Déverrouiller' : 'Verrouiller'}
      ...
      <div style={{ ...grp, padding: '4px 6px', gap: 2 }}>
        ...
        {[{ id: 'play', label: 'lecture' }, { id: 'always', label: 'toujours' }, { id: 'never', label: 'jamais' }].map(o => (
          <button key={o.id} onClick={() => setBrInPlayer(o.id)} style={{
            ...
            background: brInPlayer === o.id ? 'rgba(255,153,0,0.16)' : 'transparent',
            color: brInPlayer === o.id ? '#f90' : '#888', borderRadius: 4,
          }}>{o.label}</button>
        ))}
      </div>
      ...
      <div style={{ ...grp, padding: '4px 6px', gap: 2 }}>
        {['classique', 'neon', 'minimal'].map(s => (
          <button key={s} onClick={() => setRythmoStyle(s)} style={{
            ...
            background: rythmoStyle === s ? 'rgba(255,153,0,0.16)' : 'transparent',
            color: rythmoStyle === s ? '#f90' : '#888',
          }}>{s}</button>
        ))}
      </div>
      ...
      <div style={{ ...grp, padding: '4px 8px', gap: 6 }}>
        ...
        <input type="range" min={-2} max={2} step={0.05} value={brOffset}
          ...
          style={{ width: 80, accentColor: '#f90' }} />
        <span style={{ fontSize: 11, color: brOffset !== 0 ? '#f90' : '#888', fontFamily: 'var(--font-mono)', minWidth: 38, textAlign: 'right' }}>
          {brOffset >= 0 ? '+' : ''}{brOffset.toFixed(2)}s
        </span>
        {brOffset !== 0 && (
          <button onClick={() => setBrOffset(0)} style={{ ...btnBase, height: 20, width: 20, minWidth: 20, padding: 0, fontSize: 11, color: '#f90' }}>✕</button>
        )}
      </div>
      ...
      <div style={grp}>
        ...
        <span style={{ fontSize: 11, color: fontScale !== 1 ? '#f90' : '#888', fontFamily: 'var(--font-mono)', minWidth: 38, textAlign: 'center' }}>{Math.round(fontScale * 100)}%</span>
        <button onClick={() => setFontScale(f => Math.min(3, +(f + 0.15).toFixed(2)))} title="Texte +" style={btn(true)}>
          <span style={{ fontSize: 13, fontFamily: 'var(--font-mono)', lineHeight: 1 }}>A+</span>
        </button>
        {fontScale !== 1 && (
          <button onClick={() => setFontScale(1)} title="Réinitialiser taille" style={{ ...btn(true), minWidth: 20, width: 20, padding: 0, fontSize: 11, color: '#f90' }}>✕</button>
        )}
      </div>
      ...
      {showNote && target && (
        <div style={{
          position: 'absolute', bottom: 'calc(100% + 4px)', right: 160, zIndex: 10,
          background: '#0c0c0c', border: '1px dashed #f90', borderRadius: 4,
          padding: 8, minWidth: 280, boxShadow: '0 6px 18px rgba(0,0,0,0.7)',
        }}>
          <div style={{ fontSize: 9, color: '#f90', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>
          ...
          <textarea value={target.note || ''} onChange={e => updateTarget({ note: e.target.value })} rows={2}
            ...
            style={{ width: '100%', fontFamily: 'var(--font-ui)', fontStyle: 'italic', fontSize: 11, padding: '5px 7px', background: '#0a0a0a', color: '#f90c', border: '1px solid #f904', borderRadius: 3, resize: 'vertical' }} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <button
      ...
      style={{
        ...
        background: h ? (danger ? 'rgba(229,69,69,0.2)' : 'rgba(255,153,0,0.12)') : 'transparent',
        color: danger ? 'var(--danger)' : active ? '#f90' : 'var(--text)',
