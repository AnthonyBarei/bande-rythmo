...
def _extract_audio(src: str, dst: str, audio_idx: int):
...
async def _remux_task(path: str, remux_path: str, cache_key: tuple, status_key: str):
    try:
        a_idx = cache_key[2]
        await asyncio.to_thread(_extract_audio, path, remux_path, a_idx)
...
async def get_remux_status(
    ...
    return {"status": _remux_status.get(status_key, "unknown")}
